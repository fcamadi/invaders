...
impl Invaders {
    ...
    pub fn new() -> Self {
        ...
        for x in 0..NUM_COLS {   // initialize grid with invaders
            for y in 0..NUM_ROWS {
                if (x > 1)
                    && (x < NUM_ROWS)
                    && (y > 0)
    ...
    pub fn update(&mut self, delta: Duration) -> bool {
        ...
        if self.move_timer.ready {
            ...
            if self.direction == -1 { //left direction
                let min_x = self.army.iter().map(|invader| invader.x).min().unwrap_or(0); //find min. x coordinate
                if min_x == 0 {
                    self.direction = 1;
                    downwards = true;
                }
    ...
    pub fn invader_killed_at(&mut self, x:usize, y:usize) -> bool {
        if let Some(idx) = self  //idx: index in the army's vector
            .army
            .iter()
